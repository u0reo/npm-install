...
const restoreCachedNpm = (inputPath, primaryKey, restoreKeys) => {
  console.log('trying to restore cached NPM modules')
  console.log(`inputPath: ${inputPath}`)
  console.log(`primaryKey: ${primaryKey}`)
  console.log(`restoreKeys: ${restoreKeys}`)

  return cache
    .restoreCache([inputPath], primaryKey, restoreKeys)
    .then(cache => {
...
const saveCachedNpm = (inputPath, primaryKey) => {
...
const getCacheParams = ({
  ...
  let inputPath, restoreKeys

  if (useYarn) {
    inputPath = path.join(homeDirectory, '.cache', 'yarn')
    primaryKeySegments.unshift('yarn')
  } else {
    inputPath = npmCacheFolder
  ...
  return { inputPath, primaryKey: primaryKeySegments.join('-'), restoreKeys }
}
...
const installInOneFolder = ({
  ...
  const { inputPath, primaryKey, restoreKeys } = getCacheParams({
  ...
  return api.utils
    .restoreCachedNpm(inputPath, primaryKey, restoreKeys)
    .then(npmCacheHit => {
      return api.utils.install(opts).then(() => {
        ...
        return api.utils.saveCachedNpm(inputPath, primaryKey)
      })
    })
}
